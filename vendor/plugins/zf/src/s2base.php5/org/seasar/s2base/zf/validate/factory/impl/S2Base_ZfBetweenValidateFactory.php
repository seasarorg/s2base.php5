 ...
 * @author     klove
 */
class S2Base_ZfBetweenValidateFactory implements S2Base_ZfValidateFactory {
    const ID = 'between';
    private $instance = null;
    private $validateClassName = 'Zend_Validate_Between';
    ...
    public function getInstance($paramName, Zend_Config $config) {
        ...
        if ($config->$valKey === null) {
            throw new S2Base_ZfException("$valKey validate needs min, max value. [param : $paramName]");
        }
        if ($config->$valKey->min === null) {
            throw new S2Base_ZfException("$valKey validate needs min value. [param : $paramName]");
        }
        if ($config->$valKey->max === null) {
            throw new S2Base_ZfException("$valKey validate needs max value. [param : $paramName]");
        }
        $inclusive = true;
        if ($config->$valKey->inclusive !== null) {
            $inclusive = $config->$valKey->inclusive;
        }
        if ($this->instance === null) {
            ...
            $this->instance->setInclusive($inclusive);
        }

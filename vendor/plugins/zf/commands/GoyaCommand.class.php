...
/*
    ...
    protected function setNamesWithoutDao($name){
        $this->actionName = $name;
        $name = ucfirst($name);
        $this->actionMethodName = $this->dispatcher->formatActionName($name);
        $this->serviceInterfaceName = $name . "Service";
    ...
    protected function setNamesWithCommonsDao($name,$daoName){
        $this->actionName = $name;
        $name = ucfirst($name);
        $this->actionMethodName = $this->dispatcher->formatActionName($name);
        $this->serviceInterfaceName = $name . "Service";
    ...
    protected function finalConfirm(){
        ...
        print "  controller name         : {$this->moduleName} \n";
        print "  controller class name   : {$this->controllerClassName} \n";
        print "  action name             : {$this->actionName} \n";
        print "  action method name      : {$this->actionMethodName} \n";
        print "  action dicon file name  : {$this->actionMethodName}" . S2BASE_PHP5_DICON_SUFFIX ." \n";
        print "  action template file    : {$this->actionName}" . S2BASE_PHP5_ZF_TPL_SUFFIX . " \n";
    ...
    protected function finalConfirmWithoutDao(){
        print "\n[ generate information ] \n";
        print "  controller name         : {$this->moduleName} \n";
        print "  controller class name   : {$this->controllerClassName} \n";
        print "  action name             : {$this->actionName} \n";
    ...
    protected function prepareFilesWithCommonsDao(){
        $this->prepareActionFile();
        $this->prepareHtmlFile();
        $this->prepareActionDiconFile();
        $this->prepareServiceClassFile();
    ...
    protected function prepareFilesWithoutDao(){
        $this->prepareActionFile();
        $this->prepareHtmlFile();
        $this->prepareActionDiconFile();
        $this->prepareServiceClassFileWithoutDao();
...
*/
    ...
    protected function prepareActionDiconFile(){
        ...
        $patterns = array("/@@MODULE_NAME@@/","/@@SERVICE_CLASS@@/","/@@CONTROLLER_CLASS_NAME@@/");
        $replacements = array($this->moduleName,$this->serviceClassName,$this->controllerClassName);
        $tempContent = preg_replace($patterns,$replacements,$tempContent);
        S2Base_CommandUtil::writeFile($srcFile,$tempContent);
    }    
    ...
    protected function prepareServiceClassFile(){
        ...
        $replacements = array($this->serviceClassName,$implementsInterface,$this->daoInterfaceName,$daoProp);
        $tempContent = preg_replace($patterns,$replacements,$tempContent);
        S2Base_CommandUtil::writeFile($srcFile,$tempContent);
    }
